...
<body>
  ...
  <?php
    ...
    //Inisicamos la sesion
    session_start();
    $conexion=conexionBD();
    //Creamos una variable con la session con el usuario
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        //Creamos un voto, con el nombre del usuario y la seleccion votada
        $voto = new Voto($nombreUsuario, $seleccionVotada);
        //Comprobamos si el usuario ya ha realizado el voto
        if($voto->comprobarVoto($conexion, $nombreUsuario)){
            //Si el usuario no ha votado prodecemos a votar y abrimos la pagina de los resultados
            $voto->votar($conexion,$nombreUsuario, $seleccionVotada);
            header('Location: resultados.php');
        }else{
            //Si el usuario ya ha votado salta el ERROR
